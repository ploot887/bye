...
import seq.sequencermod.client.player.SimpleSequencePlayerClient;
...
public class SequencerMainScreen extends Screen {
    ...
    private static final boolean SHOW_DEBUG_MARKS = false;

    private static final boolean SHOW_CLICK_PROBE = false;
    private static final boolean SHOW_PREVIEW_SANITY_OVERLAY = false;

    private static final String BUILD_MARK = "BUILD A";
    private static boolean BUILD_MARK_PRINTED = false;
    private static long RENDER_LOG_LAST_MS = 0L;

    private static boolean PROBE_GLFW_LMB = false;
    private static boolean PROBE_GLFW_RMB = false;
    private static long    PROBE_LAST_CLICK_MS = 0L;
    private static double  PROBE_LAST_CLICK_X = 0.0;
    private static double  PROBE_LAST_CLICK_Y = 0.0;

    private static long    PROBE_MOVE_LAST_LOG_MS = 0L;
    private static double  PROBE_MOUSE_X = 0.0;
    private static double  PROBE_MOUSE_Y = 0.0;
    ...
    // Data
    private Map<String, SequencePreset> presets = new LinkedHashMap<>();
    ...
    private CyclingButtonWidget<Boolean> camelSaddleButton;
    ...
    public void mouseMoved(double mouseX, double mouseY) {
        PROBE_MOUSE_X = mouseX;
        PROBE_MOUSE_Y = mouseY;
        long now = System.currentTimeMillis();
        if (now - PROBE_MOVE_LAST_LOG_MS >= 1000L) {
            AecPreviewProbe.out("MOVE screen: x=%.1f y=%.1f", mouseX, mouseY);
            PROBE_MOVE_LAST_LOG_MS = now;
        }
        super.mouseMoved(mouseX, mouseY);
    }

    public static void openWithLog(MinecraftClient mc) {
        AecPreviewProbe.out("OPEN screen: creating SequencerMainScreen %s", SequencerMainScreen.class.getName());
        mc.setScreen(new SequencerMainScreen());
    }
    ...
    private final seq.sequencermod.client.ui.PreviewState disguisePreviewState = new seq.sequencermod.client.ui.PreviewState();
    private final DisguisePreviewRenderer disguisePreview = new DisguisePreviewRenderer(disguisePreviewState);

    // Список дисгайзов и выбранный id
    private java.util.List<Identifier> disguiseIds = new java.util.ArrayList<>();
    private Identifier selectedDisguiseId = null;

    // Геометрия области мини‑превью дисгайза (в правой панели превью)
    private int disguisePrevX = 0, disguisePrevY = 0, disguisePrevW = 0, disguisePrevH = 0;

    // Drag состояния для вращения дисгайза
    private boolean disguiseDragging = false;
    private Identifier draggingDisguiseId = null;
    ...
    private void drawEntityAny(net.minecraft.client.gui.DrawContext ctx, int x, int y, int scale, float rotDX, float rotDY, net.minecraft.entity.Entity e) {
        ...
        if (e instanceof net.minecraft.entity.AreaEffectCloudEntity cloud) {
            int size = Math.max(24, Math.min(48, scale));
            int left = x - size / 2;
            int top  = y - size;

            AecPreviewProbe.out("ICON AEC: enter size=%d", size);

            currentPreset.applyToEntity(cloud, custom);
            drawAecIcon(ctx, left, top, size, cloud);

            AecPreviewProbe.out("ICON AEC: done");
            return; // важно: дальше в 3D‑ветки не уходим
        }
...
// ЗАМЕНА ВЕСЬ БЛОК if (isAEC) { ... } else { ... } НА ЭТО:
    ...
    // === Camel UI helper ===
    private void addCamelControlsIfNeeded(int rightX, int y) {
        // 1) Создание (если ещё нет)
        if (camelSaddleButton == null) {
            camelSaddleButton = CyclingButtonWidget.builder((Boolean v) ->
                            Text.literal(v ? "Седло: вкл" : "Седло: выкл"))
                    .values(Boolean.FALSE, Boolean.TRUE)
                    .initially(false)
                    .build(rightX, y, 160, ROW_H, Text.literal("Седло"), (btn, value) -> {
                        try {
                            var mc = MinecraftClient.getInstance();
                            if (mc != null && mc.player != null) {
                                var st = seq.sequencermod.net.client.SimpleMorphs.getCamelState(mc.player);
                                if (st != null) {
                                    st.setSaddled(value);
                                    st.setShowReins(value);
                                }
                            }
                        } catch (Throwable ignored) {}
                    });
            this.addDrawableChild(camelSaddleButton);
        }

        // 2) Положение
        camelSaddleButton.setX(rightX);
        camelSaddleButton.setY(y);

        // 3) Видимость — только когда выбран верблюд
        boolean camelSelected = (selectedEntityType == net.minecraft.entity.EntityType.CAMEL);
        camelSaddleButton.visible = camelSelected;

        // 4) Подтягиваем актуальное состояние, если есть морф верблюда
        if (camelSelected) {
            try {
                var mc = MinecraftClient.getInstance();
                if (mc != null && mc.player != null) {
                    var st = seq.sequencermod.net.client.SimpleMorphs.getCamelState(mc.player);
                    if (st != null) camelSaddleButton.setValue(st.isSaddled());
                }
            } catch (Throwable ignored) {}
        }
    }

    // Вызывать после смены выбранной сущности в списке
    private void refreshCamelControls() {
        int contentTop = PAD + TAB_H + PAD;
        int rightX = PAD + 200 + PAD;
        int y = contentTop + 8 + 3 * ROW; // строка сразу под тремя базовыми кнопками
        addCamelControlsIfNeeded(rightX, y);
    }

    private void updateAecPresetButton(boolean visible, int x, int y) {
        if (this.aecPresetButton == null) return;
        this.aecPresetButton.visible = visible;
        if (visible) {
            this.aecPresetButton.setPosition(x, y);
        }
    }

    private void drawAecIcon(DrawContext ctx, int left, int top, int sizePx, net.minecraft.entity.AreaEffectCloudEntity cloud) {
        ...
        if (SHOW_DEBUG_MARKS) {
            debugMark(ctx, left + 4, top + 4, 0xFFFF00FF, "AEC");
        }
    }
    ...
    private net.minecraft.entity.projectile.FishingBobberEntity createFishingBobberSafe() {
        try {
            if (mc == null || mc.world == null || mc.player == null) return null;

            // Основной путь для 1.20.1: new FishingBobberEntity(PlayerEntity, World, luck, lure)
            try {
                var ctor = net.minecraft.entity.projectile.FishingBobberEntity.class.getDeclaredConstructor(
                ...
                ctor.setAccessible(true);
                var e = (net.minecraft.entity.projectile.FishingBobberEntity) ctor.newInstance(mc.player, mc.world, 0, 0);
                // позиция около (0,0,0) нам не важна — в GUI всё равно переносим матрицами
                e.setPos(0.0, 0.0, 0.0);
                return e;
            } catch (NoSuchMethodException nsme) {
                // Альтернативные маппинги: попробуем искать другие сигнатуры
                for (var c : net.minecraft.entity.projectile.FishingBobberEntity.class.getDeclaredConstructors()) {
                    var pts = c.getParameterTypes();
                    if (pts.length >= 2
                            && net.minecraft.entity.player.PlayerEntity.class.isAssignableFrom(pts[0])
                            && net.minecraft.world.World.class.isAssignableFrom(pts[1])) {
                        c.setAccessible(true);
                        // Заполним недостающие параметры нулями/дефолтами
                        Object[] args = new Object[pts.length];
                        args[0] = mc.player;
                        args[1] = mc.world;
                        for (int i = 2; i < pts.length; i++) {
                            Class<?> p = pts[i];
                            args[i] = (p == int.class || p == Integer.class) ? 0
                                    : (p == boolean.class || p == Boolean.class) ? false
                                    : null;
                        }
                        var e = (net.minecraft.entity.projectile.FishingBobberEntity) c.newInstance(args);
                        e.setPos(0.0, 0.0, 0.0);
                        return e;
                    }
                }
    ...
    private void initPreviewEntityDefaults(Entity e, boolean iconMode) {
        ...
        } else if (e instanceof net.minecraft.entity.projectile.FishingBobberEntity bobber) {
            try { e.setNoGravity(true); } catch (Throwable ignored) {}
        }
    }
    ...
    private Entity getOrCreateIconEntityAny(EntityType<?> type) {
        ...
        if (type == EntityType.FISHING_BOBBER) {
            dbg("Icon create: FISHING_BOBBER");
            Entity e = createFishingBobberSafe();
            if (e != null) {
                initPreviewEntityDefaults(e, true);
                return e;
            } else {
                dbg("Icon create failed: FISHING_BOBBER");
                return null;
            }
        }
    ...
    private Entity getOrCreatePreviewAny() {
        AecPreviewProbe.out("getOrCreatePreviewAny: start");
        AecPreviewProbe.out("getOrCreatePreviewAny: world=%s type=%s",
                String.valueOf(mc.world != null), String.valueOf(selectedEntityType != null));
        ...
        if (selectedEntityType == EntityType.FISHING_BOBBER) {
            dbg("Preview create: FISHING_BOBBER");
            Entity e = createFishingBobberSafe();
            if (e != null) {
                initPreviewEntityDefaults(e, false);
                previewEntityAny = e;
                dbg("Preview FISHING_BOBBER created OK");
                return e;
            } else {
                dbg("Preview FISHING_BOBBER create failed");
                previewEntityAny = null;
                return null;
            }
        }
...
// Позиция справа сверху. Если у тебя есть правая панель — подправь y, чтобы вписать в интерфейс
        ...
        this.addDrawableChild(
                net.minecraft.client.gui.widget.ButtonWidget.builder(
                        net.minecraft.text.Text.literal("Морфнуть"),
                        btn -> doMorphSelected()
                ).dimensions(this.width - 110, this.height - 30, 100, 20).build()
        );

        // Инициализация списка дисгайзов (Этап 2)
        this.disguiseIds = new java.util.ArrayList<>();
// Если когда‑нибудь появится источник ID — сюда можно подставить значения и отсортировать:
// this.disguiseIds.sort(java.util.Comparator.comparing(Object::toString));

        if (!this.disguiseIds.isEmpty() && this.selectedDisguiseId == null) {
            this.selectedDisguiseId = this.disguiseIds.get(0);
        }
        ...
        if (entityPreviewItems.isEmpty()) {
            ...
            entityPreviewItems.put("minecraft:fishing_bobber", Items.FISHING_ROD);
    ...
    public void tick() {
        ...
        if (!this.disguiseIds.isEmpty()) {
            // Можно крутить только выбранный, чтобы не плодить вычисления
            var only = this.selectedDisguiseId == null ? java.util.List.<Identifier>of() : java.util.List.of(this.selectedDisguiseId);
            this.disguisePreview.tickAutoRotation(only, 0.4f);
        }
    }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dx, double dy) {
        ...
        if (currentTab == Tab.MORPHS && this.disguiseDragging && this.draggingDisguiseId != null) {
            this.disguisePreviewState.applyDrag(this.draggingDisguiseId, dx, dy);
            return true;
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        // Общий входной лог клика по экрану — помогает понять, что метод реально вызывается
        AecPreviewProbe.out("CLICK screen: enter x=%.1f y=%.1f btn=%d tab=%s", mouseX, mouseY, button, currentTab);

        debugHitTest(mouseX, mouseY);
        // Оверлеи и блокирующие слои — оставляем как было
        if (hotkeysOverlayVisible) {
        ...
        // Клик в поле поиска морфов — оставляем как было
        ...
        // Отдаём клики детям на вкладке MORPHS (MorphList, MorphEntry) ПЕРЕД остальными зонами
        if (currentTab == Tab.MORPHS) {
            if (super.mouseClicked(mouseX, mouseY, button)) {
                AecPreviewProbe.out("CLICK screen: handled by super (children)");
                return true;
            }
        }

        // Остальная логика как была (PRESETS: резайзеры, попапы, таймлайн; MORPHS: превью и т.д.)
        if (currentTab == Tab.PRESETS && button == 0) {
        ...
        if (currentTab == Tab.MORPHS && button == 0 && this.selectedDisguiseId != null) {
            if (mouseIn(mouseX, mouseY, this.disguisePrevX, this.disguisePrevY, this.disguisePrevW, this.disguisePrevH)) {
                this.disguiseDragging = true;
                this.draggingDisguiseId = this.selectedDisguiseId;
                return true;
            }
        }
        ...
        // Попап автодополнения ID (PRESETS) — оставляем как было
        if (idAutocompleteVisible && entityIdBox != null && entityIdBox.isFocused()) {
            if (mouseIn(mouseX, mouseY, idPopupX, idPopupY, idPopupW, idPopupH)) {
                int relY = (int)(mouseY - idPopupY) - 3;
        ...
        // Для прочих случаев — стандартная передача наверх
        AecPreviewProbe.out("CLICK screen: pass to super");
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        ...
        if (currentTab == Tab.MORPHS && button == 0 && this.disguiseDragging) {
            this.disguiseDragging = false;
            this.draggingDisguiseId = null;
            return true;
        }
...
// Возвращает true, если граница найдена и мы вошли в режим edge‑adjust.
    ...
    private void debugHitTest(double mouseX, double mouseY) {
        try {
            boolean any = false;
            for (var e : this.children()) {
                boolean over = false;
                try {
                    over = e.isMouseOver(mouseX, mouseY);
                } catch (Throwable ignored) {}
                if (over) {
                    any = true;
                    AecPreviewProbe.out("HIT child: %s", e.getClass().getName());
                    // Если это кнопка — выведем геометрию
                    if (e instanceof net.minecraft.client.gui.widget.ClickableWidget cw) {
                        AecPreviewProbe.out("HIT button bounds: x=%d y=%d w=%d h=%d",
                                cw.getX(), cw.getY(), cw.getWidth(), cw.getHeight());
                    }
                }
            }
            if (!any) {
                AecPreviewProbe.out("HIT child: none");
            }
            // Если есть поле morphList — отметим попадание в область списка
            try {
                var f = this.getClass().getDeclaredField("morphList");
                f.setAccessible(true);
                Object list = f.get(this);
                if (list instanceof net.minecraft.client.gui.Element el) {
                    boolean overList = el.isMouseOver(mouseX, mouseY);
                    AecPreviewProbe.out("HIT morphList: over=%s (%s)", overList, list.getClass().getName());
                }
            } catch (Throwable ignored) {}
        } catch (Throwable t) {
            AecPreviewProbe.out("debugHitTest error: %s", String.valueOf(t));
        }
    }
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        AecPreviewProbe.nextFrame();
        long nowMs = System.currentTimeMillis();
        if (nowMs - RENDER_LOG_LAST_MS >= 1000L) {
            AecPreviewProbe.out("RENDER screen: class=%s tab=%s", this.getClass().getName(), currentTab);
            RENDER_LOG_LAST_MS = nowMs;
        }

// Жёсткая метка сборки — печатается один раз за сессию
        if (!BUILD_MARK_PRINTED) {
            AecPreviewProbe.out("frame 1 (BUILD %s) — %s", BUILD_MARK, this.getClass().getName());
            BUILD_MARK_PRINTED = true;
        }

        try {
            long win = MinecraftClient.getInstance().getWindow().getHandle(); // используем гарантированно валидный доступ
            boolean lmbNow = org.lwjgl.glfw.GLFW.glfwGetMouseButton(win, org.lwjgl.glfw.GLFW.GLFW_MOUSE_BUTTON_LEFT)  == org.lwjgl.glfw.GLFW.GLFW_PRESS;
            boolean rmbNow = org.lwjgl.glfw.GLFW.glfwGetMouseButton(win, org.lwjgl.glfw.GLFW.GLFW_MOUSE_BUTTON_RIGHT) == org.lwjgl.glfw.GLFW.GLFW_PRESS;

            if (lmbNow != PROBE_GLFW_LMB) {
                PROBE_GLFW_LMB = lmbNow;
                if (lmbNow) {
                    PROBE_LAST_CLICK_MS = System.currentTimeMillis();
                    PROBE_LAST_CLICK_X  = mouseX;
                    PROBE_LAST_CLICK_Y  = mouseY;
                    AecPreviewProbe.out("GLFW LMB DOWN at x=%.1f y=%.1f", (double) mouseX, (double) mouseY);
                } else {
                    AecPreviewProbe.out("GLFW LMB UP   at x=%.1f y=%.1f", (double) mouseX, (double) mouseY);
                }
            }
            if (rmbNow != PROBE_GLFW_RMB) {
                PROBE_GLFW_RMB = rmbNow;
                if (rmbNow) {
                    PROBE_LAST_CLICK_MS = System.currentTimeMillis();
                    PROBE_LAST_CLICK_X  = mouseX;
                    PROBE_LAST_CLICK_Y  = mouseY;
                    AecPreviewProbe.out("GLFW RMB DOWN at x=%.1f y=%.1f", (double) mouseX, (double) mouseY);
                } else {
                    AecPreviewProbe.out("GLFW RMB UP   at x=%.1f y=%.1f", (double) mouseX, (double) mouseY);
                }
            }
        } catch (Throwable t) {
            AecPreviewProbe.out("GLFW PROBE ERROR: %s", String.valueOf(t));
        }

// === PROBE: рисуем маленькую красную точку на экране в месте последнего клика (видна ~1.2 сек) ===
        if (System.currentTimeMillis() - PROBE_LAST_CLICK_MS < 1200L) {
            ctx.getMatrices().push();
            ctx.getMatrices().translate(0, 0, 600); // поверх всего
            int cx = (int) Math.round(PROBE_LAST_CLICK_X);
            int cy = (int) Math.round(PROBE_LAST_CLICK_Y);
            ctx.fill(cx - 2, cy - 2, cx + 2, cy + 2, 0xFFFF4444);
            ctx.getMatrices().pop();
        }

        AecPreviewProbe.out("Screen.render tab=%s", currentTab);
        AecPreviewProbe.out("Render: selectedType=%s",
                selectedEntityType == null ? "null"
                        : String.valueOf(net.minecraft.registry.Registries.ENTITY_TYPE.getId(selectedEntityType)));
        AecPreviewProbe.rect("PreviewRect",
                morphPrevLeft, morphPrevTop,
                Math.max(0, morphPrevRight - morphPrevLeft),
                Math.max(0, morphPrevBottom - morphPrevTop));
        ...
        if (currentTab == Tab.MORPHS) {
            ...
            if (searchBox != null) {
                ...
                if (SHOW_PREVIEW_SANITY_OVERLAY) {
                    try {
                        // Полупрозрачная подложка, чтобы видеть границы зоны
                        int bg = 0x402266FF;
                        ctx.fill(morphPrevLeft, morphPrevTop, morphPrevRight, morphPrevBottom, bg);

                        int areaW = Math.max(16, morphPrevRight - morphPrevLeft);
                        int areaH = Math.max(16, morphPrevBottom - morphPrevTop);
                        int cx = morphPrevLeft + areaW / 2;
                        int cy = morphPrevTop + areaH - 10;
                        int size = Math.max(10, Math.min(areaW, areaH) / 2);

                        ctx.getMatrices().push();
                        ctx.getMatrices().translate(0, 0, 600); // поверх всех 2D-слоёв
                        if (this.mc != null && this.mc.player != null) {
                            InventoryScreen.drawEntity(ctx, cx, cy, size, 0f, 0f, this.mc.player);
                            AecPreviewProbe.out("PREVIEW sanity: drew player at cx=%d cy=%d size=%d", cx, cy, size);
                        } else {
                            AecPreviewProbe.out("PREVIEW sanity: no player");
                        }
                        ctx.getMatrices().pop();
                    } catch (Throwable t) {
                        AecPreviewProbe.out("PREVIEW sanity error: %s", String.valueOf(t));
                    }
                }
                ctx.getMatrices().pop();
            }
            ...
            int rightX = PAD + 200 + PAD;
                ...
                int btnCount = (selectedEntityType == net.minecraft.entity.EntityType.CAMEL) ? 4 : 3;
            ...
            if (aecPresetButton != null) {
                boolean aec = (selectedEntityType == net.minecraft.entity.EntityType.AREA_EFFECT_CLOUD);
                int btnX = previewRight - aecPresetButton.getWidth();
                int btnY = Math.max(PAD, previewTop - aecPresetButton.getHeight() - 4);
                updateAecPresetButton(aec && currentTab == Tab.MORPHS, btnX, btnY);

                if (this.selectedDisguiseId != null) {
                    // Размер мини‑окна: до трети ширины общей области превью, макс ~110 px
                    int availW = Math.max(16, previewRight - previewLeft);
                    int availH = Math.max(16, previewBottom - previewTop);
                    int dSize = Math.min(110, Math.min(availW / 3, availH / 2));
                    dSize = Math.max(48, dSize);

                    // Позиция — 8 px отступ от левого верхнего угла общей зоны превью
                    this.disguisePrevX = previewLeft + 8;
                    this.disguisePrevY = previewTop + 8;
                    this.disguisePrevW = dSize;
                    this.disguisePrevH = dSize;

                    // Рендер дисгайза в рамке
                    this.disguisePreview.renderMain(this.selectedDisguiseId, ctx, this.disguisePrevX, this.disguisePrevY, this.disguisePrevW, this.disguisePrevH, delta);

                    // Необязательная подпись под мини‑превью (сокращённый id)
                    try {
                        String sid = this.selectedDisguiseId.toString();
                        int sw = textRenderer.getWidth(sid);
                        int tx = this.disguisePrevX + (this.disguisePrevW - sw) / 2;
                        int ty = this.disguisePrevY + this.disguisePrevH + 2;
                        ctx.drawText(textRenderer, Text.literal(sid), tx, ty, 0xCCCCCC, false);
                    } catch (Throwable ignored) {}
                }
            }
...
// Любой Entity для превью
            ...
            if (selectedEntityType == EntityType.AREA_EFFECT_CLOUD) {
                int pw = Math.max(16, morphPrevRight - morphPrevLeft);
                int ph = Math.max(16, morphPrevBottom - morphPrevTop);

                AreaEffectCloudEntity cloud = null;
                if (toRenderAny instanceof AreaEffectCloudEntity c) {
                    cloud = c;
                } else {
                    cloud = createAreaEffectCloudSafe();
                }
                ...
                if (cloud != null) {
                    ...
                    try {
                        ...
                        AecPreviewProbe.out("PREVIEW AEC fallback (render2DInRect failed): %s", String.valueOf(t));
                        drawAreaEffectCloudOverlay(ctx, morphPrevLeft + pw / 2, morphPrevTop + ph / 2, Math.min(pw, ph) / 2, cloud);
                    }
                } else {
                    AecPreviewProbe.out("PREVIEW AEC (forced): cloud==null, skip");
                }

                // подпись id
                Identifier sid = Registries.ENTITY_TYPE.getId(selectedEntityType);
                if (sid != null) {
                    String s = sid.toString();
                    int sw = textRenderer.getWidth(s);
                    int pX = morphPrevLeft + (pw / 2);
                    ctx.drawText(textRenderer, Text.literal(s), pX - sw / 2, morphPrevBottom - 16, 0xAAAAAA, false);
                }
                return; // важно: не идём в стандартные ветки ниже
            }
            ...
            if (toRenderAny != null) {
                ...
                try {
                    if (toRenderAny instanceof net.minecraft.entity.AreaEffectCloudEntity cloud) {
                        AecPreviewProbe.out("PREVIEW AEC: start");
                        currentPreset.applyToEntity(cloud, custom);

                        int pw = Math.max(16, morphPrevRight - morphPrevLeft);
                        int ph = Math.max(16, morphPrevBottom - morphPrevTop);

                        AecPreviewProbe.rect("PREVIEW AEC: rect", morphPrevLeft, morphPrevTop, pw, ph);
                        ...
                        AecPreviewProbe.gl("PREVIEW AEC: gl-after");
                        AecPreviewProbe.out("PREVIEW AEC: done");

                    } else if (toRenderAny instanceof LivingEntity le) {
                        AecPreviewProbe.out("PREVIEW draw begin: type=%s rect=%d,%d %dx%d",
                                String.valueOf(Registries.ENTITY_TYPE.getId(le.getType())),
                                morphPrevLeft, morphPrevTop,
                                (morphPrevRight - morphPrevLeft), (morphPrevBottom - morphPrevTop));
                        InventoryScreen.drawEntity(ctx, pX, pY, scale, rotDX, rotDY, le);
                        AecPreviewProbe.out("PREVIEW draw end");
                    } else {
    ...
    private void doMorphSelected() {
        if (this.client == null || this.client.player == null) {
            setStatusSafe("Нет локального игрока");
            return;
        }
        if (selectedEntityType == null) {
            setStatusSafe("Не выбрана сущность для морфа");
            return;
        }
        var id = net.minecraft.registry.Registries.ENTITY_TYPE.getId(selectedEntityType);
        if (id == null) {
            setStatusSafe("Неизвестный id сущности");
            return;
        }

        try {
            var me = this.client.player.getUuid();

            // Тоггл: если уже выбран этот же морф — просим сервер снять
            var current = seq.sequencermod.net.client.MorphClientSync.getMorphType(me);
            if (current != null && current.equals(id)) {
                seq.sequencermod.net.SequencerNetworkingClient.requestMorphC2S(null);
                setStatusSafe("Морф снят");
                return;
            }

            // Просим сервер применить морф (или локально, если оффлайн)
            seq.sequencermod.net.SequencerNetworkingClient.requestMorphC2S(id);
            setStatusSafe("Морф запрошен: " + id);
            if ("minecraft".equals(id.getNamespace()) && "allay".equals(id.getPath())) {
                if (this.client != null && this.client.player != null) {
                    seq.sequencermod.net.client.morphs.MorphRuntimeFlags.setAllayMorph(this.client.player, true);
                }
            }
        } catch (Throwable t) {
            t.printStackTrace();
            setStatusSafe("Ошибка морфа: " + t.getClass().getSimpleName()
                    + (t.getMessage() != null ? (" — " + t.getMessage()) : ""));
        }
    }

    // Утилита для статуса (если у тебя нет setStatus)
    private void setStatusSafe(String msg) {
        try {
            // если в твоём классе есть свой setStatus(...) — зови его
            this.setStatus(msg);
        } catch (Throwable t) {
            if (this.client != null && this.client.player != null) {
                this.client.player.sendMessage(net.minecraft.text.Text.literal(msg), false);
            } else {
                System.out.println("[Sequencer|Client] " + msg);
            }
        }
    }
    ...
    private void ensurePresetLoaded() {
        if (presets == null || presets.isEmpty()) {
            Map<String, SequencePreset> loaded = PresetStorage.load();
            if (loaded == null) loaded = new LinkedHashMap<>();
            presets = loaded;

            if (selectedPresetName == null && !presets.isEmpty()) {
                selectedPresetName = presets.keySet().iterator().next();
            }
            // гарантируем список комментариев для каждого пресета
            for (String name : presets.keySet()) {
                presetComments.putIfAbsent(name, new ArrayList<>());
            }
        }
    }
    ...
    private class MorphList extends AlwaysSelectedEntryListWidget<MorphEntry> {
        ...
        @Override
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            boolean inside = mouseY >= this.top && mouseY <= this.bottom
                    && mouseX >= this.getRowLeft() && mouseX <= (this.getRowLeft() + this.width);
            AecPreviewProbe.out("CLICK screen: x=%.1f y=%.1f btn=%d tab=%s", mouseX, mouseY, button, currentTab);
            AecPreviewProbe.out("MorphList.mouseClicked: inside=%s x=%.1f y=%.1f btn=%d",
                    String.valueOf(inside), mouseX, mouseY, button);

            MorphEntry over = this.getEntryAtPosition(mouseX, mouseY);
            AecPreviewProbe.out("MorphList.mouseClicked: over=%s", (over != null ? over.display : "null"));

            boolean handled = super.mouseClicked(mouseX, mouseY, button);
            AecPreviewProbe.out("MorphList.mouseClicked: handled=%s selected=%s",
                    String.valueOf(handled),
                    (getSelectedOrNull() != null ? getSelectedOrNull().display : "null"));
            return handled;
        }

        @Override
        public void setSelected(MorphEntry entry) {
            super.setSelected(entry);
            if (entry != null) {
                // Обновляем выбранный тип всегда при смене выделения
                selectedEntityType = entry.type;

                // «Разбанить» на случай прошлых падений
                if (selectedEntityType == net.minecraft.entity.EntityType.AREA_EFFECT_CLOUD
                        || selectedEntityType == net.minecraft.entity.EntityType.EVOKER_FANGS) {
                    unbanCloudAndFangsFromBadCaches();
                }

                // Проба
                AecPreviewProbe.out("Morph selected via setSelected: %s", entry.display);

                // Обновляем видимость/состояние кнопки седла
                SequencerMainScreen.this.refreshCamelControls();
            }
        }
        ...
        void rebuildEntries() {
            ...
            if (getSelectedOrNull() == null && !children().isEmpty()) {
    ...
    private class MorphEntry extends AlwaysSelectedEntryListWidget.Entry<MorphEntry> {
        ...
        @Override
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            AecPreviewProbe.out("MorphEntry.mouseClicked: %s x=%.1f y=%.1f btn=%d", this.display, mouseX, mouseY, button);

            // Выделяем строку и проставляем выбранный тип
            parent.setSelected(this);
            selectedEntityType = type;
            AecPreviewProbe.out("Morph clicked: %s", this.display);

            // Сброс и пересоздание превью-entity, если нужно
            if (previewEntity != null) { previewEntity.discard(); previewEntity = null; }

            // Разбан кэшей для AEC/Fangs
            if (selectedEntityType == net.minecraft.entity.EntityType.AREA_EFFECT_CLOUD
                    || selectedEntityType == net.minecraft.entity.EntityType.EVOKER_FANGS) {
                unbanCloudAndFangsFromBadCaches();
            }

            // Восстановление поз/зума из кэша
            Identifier id = Registries.ENTITY_TYPE.getId(selectedEntityType);
            if (id != null) {
            ...
            // Двойной клик ЛКМ — действуем как раньше
            if (button == 0) {
                long now = System.currentTimeMillis();
