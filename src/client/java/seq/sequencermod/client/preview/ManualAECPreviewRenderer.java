...
public final class ManualAECPreviewRenderer {
    ...
    private static void blitFbo(DrawContext ctx, SimpleFramebuffer fb, int x, int y, int w, int h) {
        AecPreviewProbe.out("MAEC BLIT: dst=(%d,%d %dx%d)", x, y, w, h);
        AecPreviewProbe.gl("MAEC BLIT: gl-before");
        ...
        AecPreviewProbe.gl("MAEC BLIT: gl-after");
    ...
    public static void render2DInRect(DrawContext ctx,
        ...
        MatrixStack matrices = ctx.getMatrices();
        AecPreviewProbe.out("MAEC 2D: enter x=%d y=%d w=%d h=%d", x, y, w, h);
        AecPreviewProbe.style("MAEC 2D: style", style);
        AecPreviewProbe.gl("MAEC 2D: gl-before");
        matrices.push();
        matrices.translate(0, 0, 400);
        ...
        AecPreviewProbe.gl("MAEC 2D: gl-after");
        AecPreviewProbe.out("MAEC 2D: leave");
    ...
    // ЗАМЕНИ свой метод на этот
    private static Sprite[] getEffectFrames() {
        if (EFFECT_FRAMES_CACHE != null) return EFFECT_FRAMES_CACHE;

        // [AEC][PROBE] — начало загрузки кадров
        AecPreviewProbe.out("MAEC frames: load");
        ...
        int loaded = 0; // ВАЖНО: этот счётчик должен быть объявлен ДО использования в логе
        ...
        // [AEC][PROBE] — логируем СКОЛЬКО реально загрузили, СТРОГО ПОСЛЕ цикла и до fallback
        AecPreviewProbe.out("MAEC frames: loaded=%d", loaded);

        if (loaded == 0) {
            // [AEC][PROBE] — эта строка должна стоять ПЕРВОЙ внутри fallback
            AecPreviewProbe.out("MAEC frames: fallback used");
